  ...
  <div class="w-full">
    ...
    <div class="grid grid-cols-7" style="border-left: 1px solid #e5e5e5" role="grid" aria-label="日历">
      <div
        ...
        :style="{
          backgroundColor: day.date.format('YYYY-MM-DD') === today
            ? '#f0f0f0'
            : (day.isCurrentMonth
              ? ((day.date.day() === 0 || day.date.day() === 6) ? '#fafafa' : '#fff')
              : '#f5f5f5'),
        }"
        @mouseenter="$event.currentTarget.style.backgroundColor = day.date.format('YYYY-MM-DD') === today ? '#ebebeb' : '#f5f5f5'"
        @mouseleave="$event.currentTarget.style.backgroundColor = day.date.format('YYYY-MM-DD') === today ? '#f0f0f0' : (day.isCurrentMonth ? ((day.date.day() === 0 || day.date.day() === 6) ? '#fafafa' : '#fff') : '#f5f5f5')"
        role="gridcell"
        :tabindex="day.isCurrentMonth ? 0 : -1"
        @click="emit('select-date', day.date.format('YYYY-MM-DD'))"
        @keyup.enter="emit('select-date', day.date.format('YYYY-MM-DD'))"
      >
        <!-- 日期数字 + 添加按钮 -->
        <div class="flex items-center justify-between mb-1 md:mb-1.5">
          <span
            class="text-xs md:text-sm"
            :class="day.date.format('YYYY-MM-DD') === today ? 'today-badge' : ''"
            :style="{
              color: day.date.format('YYYY-MM-DD') === today ? '#fff' : (day.isCurrentMonth ? '#555' : '#ccc'),
              fontWeight: day.date.format('YYYY-MM-DD') === today ? '600' : '400',
          ...
          <button
            ...
            aria-label="添加事项"
            @click.stop="emit('add-event', day.date.format('YYYY-MM-DD'))"
        ...
        <div class="hidden md:flex flex-col gap-1">
          <div
            v-for="(tag, i) in getGameTags(day.date.format('YYYY-MM-DD')).slice(0, maxShow)"
            :key="i"
          ...
          <div
            v-if="getGameTags(day.date.format('YYYY-MM-DD')).length > maxShow"
            style="font-size: 0.6875rem; color: #999; padding-left: 6px"
          >
            +{{ getGameTags(day.date.format('YYYY-MM-DD')).length - maxShow }} 更多
          </div>
        </div>
        ...
        <div class="flex md:hidden flex-wrap gap-1 mt-1">
          <span
            v-for="(tag, i) in getGameTags(day.date.format('YYYY-MM-DD')).slice(0, 5)"
            :key="i"
          ...
          ></span>
          <span
            v-if="getGameTags(day.date.format('YYYY-MM-DD')).length > 5"
            style="font-size: 0.5625rem; color: #999"
          >+{{ getGameTags(day.date.format('YYYY-MM-DD')).length - 5 }}</span>
        </div>
      </div>
    </div>
  </div>
